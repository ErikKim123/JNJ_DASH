...
import { JudgeBadge } from '@/components/vote/JudgeBadge';
import { LoadingSkeleton } from '@/components/vote/LoadingSkeleton';
import { NavBar } from '@/components/vote/NavBar';
import {
  PassFailToggle,
  type RowStatus,
} from '@/components/vote/PassFailToggle';
import { ScoreInput, isValidScore } from '@/components/vote/ScoreInput';
import { ToastViewport, useToasts } from '@/components/vote/Toast';
import { useCompetition } from '@/hooks/vote/useCompetition';
import { useDraft } from '@/hooks/vote/useDraft';
import { useJudge } from '@/hooks/vote/useJudge';
import {
  AppsScriptError,
  getEvent,
  getJudges,
...
function PassFailBody({
  ...
  function handleSubmit() {
    ...
    submitRound({ judgeId, round, entries }, sheetId, true)
      .then((res) => {
        setSubmitState({ kind: 'locked' });
        push('success', `Submitted ${res.written}.`);
        ...
        secondary={
          locked
            ? {
                ...
                onClick: () => {
                  setSubmitState({ kind: 'idle' });
                  // 어드민 녹색 해제 — 제출 플래그만 내린다(저장된 점수는 유지).
                  submitRound({ judgeId, round, entries: [] }, sheetId, false).catch(
                    () => {},
                  );
                },
              }
...
function FinalBody({
  ...
  function handleSubmit() {
    ...
    setTimeout(() => {
      ...
      submitRound({ judgeId, round: 'final', entries }, sheetId, true)
        .then((res) => {
          push('success', `Submitted (${res.written}).`);
          setSubmitState({ kind: 'locked' });
          clear();
...
function AvatarImg({ url, number, size }: { url: string; number: string; size: number }) {
  ...
  // lh3 URL → ID extraction (the /api/vote/round route already normalized Drive
