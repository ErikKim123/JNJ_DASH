...
import { setCompetition, useCompetition } from '@/hooks/vote/useCompetition';
import { setJudge } from '@/hooks/vote/useJudge';
import type { Competition, Judge } from '@/lib/vote/sheet-schema';
...
function EnterPageInner() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    fetch('/api/vote/competitions', { cache: 'no-store' })
      .then(async (res) => {
        ...
        if (match) {
          ...
          window.location.replace('/vote/enter');
          return;
        }
        setQrSyncing(false);
      })
  ...
  useEffect(() => {
    ...
    const url = `/api/vote/judges?competitionId=${encodeURIComponent(competition.id)}`;
...
function JudgeLoginQR({ competitionId }: { competitionId: string }) {
  ...
  const judgeUrl =
    typeof window !== 'undefined'
      ? `${window.location.origin}/vote/enter?c=${encodeURIComponent(competitionId)}`
      : `/vote/enter?c=${encodeURIComponent(competitionId)}`;
