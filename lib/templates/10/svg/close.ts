...
function seal(cx: number, cy: number): string {
  ...
  return `
    <g>
      ${rings}
      <circle cx="${cx}" cy="${cy}" r="${r}" fill="url(#t10CardFill)" stroke="url(#t10AccentStroke)" stroke-width="3"/>
      ...
      <path d="M ${f(cx - 34)} ${f(cy + 2)} L ${f(cx - 10)} ${f(cy + 27)} L ${f(cx + 38)} ${f(cy - 26)}" fill="none"
        stroke="url(#t10AccentNum)" stroke-width="11" stroke-linecap="round" stroke-linejoin="round"
