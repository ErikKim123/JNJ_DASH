// Template 10 — RESULT (예선/본선 통과자 명단).
...
import {
  shell, topBar, footBar, heading, waveCard, label, accentNumber, strong, photoFrame, fadeUp, clipBox,
...
function chipCard(key: Key, i: number, x: number, y: number, w: number, h: number): string {
  ...
  return `
    ${waveCard(x, y, w, h, { rx: Math.min(14, h * 0.25) })}
