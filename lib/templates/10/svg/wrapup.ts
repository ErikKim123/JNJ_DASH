// Template 10 — WRAPUP (집계 중). final=true 면 결승 변형(무대 조명 + 강조 띠).
// 무대 띠 안에 슬롯머신처럼 굴러가는 강조 숫자 릴 7개 — "점수를 계산하는 중"을 글자 없이 보여준다.
// 릴마다 속도·시작 위치가 달라 기계적으로 똑같이 돌지 않는다.
import {
  shell, topBar, footBar, stageBand, waveCard, label, strong, clipBox, lightRays, seeded,
  DISPLAY, SOFT, ACCENT, CX, MX, RX, f,
} from '../common';

const REEL_DEFS = `
  <defs>
    <linearGradient id="t10ReelFade" x1="0" y1="0" x2="0" y2="1">
      <stop offset="0" stop-color="#0A1648" stop-opacity="1"/>
      <stop offset="0.14" stop-color="#0A1648" stop-opacity="0.85"/>
      <stop offset="0.38" stop-color="#0A1648" stop-opacity="0"/>
      <stop offset="0.62" stop-color="#0A1648" stop-opacity="0"/>
      <stop offset="0.86" stop-color="#0A1648" stop-opacity="0.85"/>
      <stop offset="1" stop-color="#0A1648" stop-opacity="1"/>
    </linearGradient>
  </defs>
`;
...
function reel(x: number, y: number, w: number, h: number, idx: number, gold: boolean): string {
  ...
  for (let k = 0; k <= 10; k++) {
    digits += `<text x="${f(cx)}" y="${f(base + k * step)}" text-anchor="middle" font-family="${DISPLAY}" font-weight="900"
      font-size="${f(size)}" fill="url(#t10AccentNum)">${k % 10}</text>`;
  }
  ...
  return `
    ${waveCard(x, y, w, h, { rx: 16, hero: gold })}
    ${clipBox(x + 3, y + 3, w - 6, h - 6, strip)}
    <rect x="${f(x + 3)}" y="${f(y + 3)}" width="${f(w - 6)}" height="${f(h - 6)}" rx="13" fill="url(#t10ReelFade)"/>
