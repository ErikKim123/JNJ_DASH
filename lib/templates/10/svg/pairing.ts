...
import {
  shell, topBar, footBar, heading, waveCard, label, accentNumber, strong, roleChip, fadeUp, clipBox,
...
function renderCircle(pairCount: number): string {
  ...
  const center = `
    <g opacity="0">
      ...
      <circle cx="${f(L.cx)}" cy="${f(L.cy)}" r="${r}" fill="url(#t10CardFill)" stroke="url(#t10AccentStroke)" stroke-width="2.4"/>
