// Template 11 — 결승 전용 화면 (Prep / Wrapup / Result / Pairing).
import { prepSvg } from './prep';
import { wrapupSvg } from './wrapup';
import {
  shell, topBar, footBar, heading, panelCard, label, accentNumber, strong, rolePill, rankMark, photoFrame,
  ...
  const mystery = `
    <g class="t07-mystery">
      ...
      <text class="num" x="${f(cx)}" y="${f(photoY + ph / 2 + (big ? 38 : 31))}" text-anchor="middle"
        font-family="'Montserrat', 'Arial Black', sans-serif" font-weight="900" font-size="${big ? 108 : 88}" fill="url(#t11AccentNum)" opacity="0.45">?<animate
...
export function finalResultSvg(): string {
  return shell(`
    ...
    <rect x="${CX - 0.75}" y="220" width="1.5" height="410" fill="url(#t11RuleV)"/>
    ${footBar()}
  `);
}
