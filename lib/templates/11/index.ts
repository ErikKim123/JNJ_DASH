// Template 11 — "SOLAR SLASH".
// 거의 검은 틸 배경을 주황-금빛 사선 광선이 가르고, 광선을 따라 빛줄기가 흩어지며,
// 대진 화면에서는 그 위로 어두운 실루엣 "VS"가 역광으로 떠오른다.
// 잭앤질 스텝 화면·흐름은 07(LIVE ARENA)과 같고 11/common.ts 의 시각 프리미티브만 다르다.
...
const REPORT_THEME = {
  ...
  mono: DISPLAY,
  italic: false,
  // 11 은 상단 러너(대회명·아이콘·라이브 뱃지)가 y 84 까지 쓰고 하단 러너가 y 660 부터 시작한다.
  // 표를 그 사이(192~644)에 넣고, 좌우는 11 의 여백(32)에 맞춰 넓게 편다.
  layout: {
    leftX: 32,
    rightX: 652,
    colW: 596,
...
/**
 * 배경 레이어 확정.
 *  · 커스텀 배경이 없으면 11 기본 배경(벡터 연출)을 깐다 (그 아래 바탕 그라디언트는 shell 이 항상 깐다).
 *  · 있으면 기본 배경을 빼고 운영자 이미지 + 스크림을 깐다 — 11 은 흰 글자라
...
export const Template11: TemplateModule = {
  id: 11,
  name: 'Solar Slash — 금빛 사선 VS (11)',
