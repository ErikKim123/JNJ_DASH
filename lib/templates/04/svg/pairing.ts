...
function renderCirclePairing(pairCount: number): string {
  ...
  const centerMark = `
    <g transform="translate(${cx} ${cy})" opacity="0">
      ...
      <text text-anchor="middle" y="-8" font-family="'Cormorant Garamond', Georgia, serif" font-size="22" letter-spacing="6" fill="#DDAE42" opacity="0.55">✦</text>
      <line x1="-34" y1="10" x2="34" y2="10" stroke="url(#goldgh)" stroke-width="0.7" opacity="0.65"/>
      <text text-anchor="middle" y="28" font-family="'Cormorant Garamond', Georgia, serif" font-style="italic" font-size="11" letter-spacing="5" fill="#DDAE42" opacity="0.55">stage</text>
    </g>
  `;
    ...
    body += `
      <g opacity="0">
        ...
        <g transform="translate(${px.toFixed(1)} ${py.toFixed(1)})" font-family="'Cormorant Garamond', Georgia, 'Gulim', '굴림', serif">
          <text text-anchor="${anchor}" y="${leaderY}" font-family="${roleFont}" font-size="${roleFontSize}" font-weight="700" letter-spacing="0.5" fill="#F5C64E">L{{leader_num_${i}}}</text>
          <text text-anchor="${anchor}" y="${followerY}" font-family="${roleFont}" font-size="${roleFontSize}" font-weight="700" letter-spacing="0.5" fill="url(#silvergt)">F{{follower_num_${i}}}</text>
  ...
  return shell(`
    ${topHeader()}
    <text x="640" y="146" text-anchor="middle" font-family="'Cinzel', 'Cormorant Garamond', Georgia, 'Gulim', '굴림', serif" font-weight="bold" font-size="36" letter-spacing="10" fill="url(#goldgt)">{{round_title}}</text>
    <text x="640" y="176" text-anchor="middle" font-family="'Cormorant Garamond', Georgia, 'Gulim', '굴림', serif" font-style="italic" font-size="15" letter-spacing="6" fill="#F7F4EA" opacity="0.85">{{stage_label}}</text>
    ...
    rows += `
      <g opacity="0">
        ...
        <text x="${(tx + trackW * 0.62).toFixed(1)}" y="${baseline.toFixed(1)}" text-anchor="end" font-family="${roleFont}"
          font-size="${fs.toFixed(1)}" font-weight="700" letter-spacing="0.5" fill="#F5C64E">L{{leader_num_${i}}}</text>
        ...
        <text x="${(tx + trackW * 0.72).toFixed(1)}" y="${baseline.toFixed(1)}" text-anchor="start" font-family="${roleFont}"
          font-size="${fs.toFixed(1)}" font-weight="700" letter-spacing="0.5" fill="url(#silvergt)">F{{follower_num_${i}}}</text>
  ...
  const colHeaders = Array.from({ length: cols }, (_, c) => {
    ...
    return `
      <g transform="translate(0 ${(top - 16).toFixed(1)})">
        <text x="${(tx + trackW * 0.62).toFixed(1)}" y="0" text-anchor="end" font-family="${monoFont}" font-size="10" letter-spacing="4" fill="#DDAE42">{{label_leader}}</text>
        <text x="${(tx + trackW * 0.72).toFixed(1)}" y="0" text-anchor="start" font-family="${monoFont}" font-size="10" letter-spacing="4" fill="#DDAE42">{{label_follower}}</text>
      </g>
    `;
  }).join('');
