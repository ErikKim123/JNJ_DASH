...
export function ceremonySvg(): string {
  ...
  const rankLabel = (cx: number, cy: number, text: string, fontSize = 16, color = '#DDAE42') => {
  ...
  return shell(`
    ...
    <text x="640" y="146" text-anchor="middle" font-family="'Cinzel', 'Cormorant Garamond', Georgia, 'Gulim', '굴림', serif" font-weight="bold" font-size="46" letter-spacing="12" fill="url(#goldgt)">{{ceremony_title}}</text>
    <text x="640" y="178" text-anchor="middle" font-family="'Cormorant Garamond', Georgia, 'Gulim', '굴림', serif" font-style="italic" font-size="17" letter-spacing="6" fill="#F7F4EA" opacity="0.9">{{ceremony_subtitle}}</text>

    ${rankLabel((firstLeaderX + firstFollowerX) / 2, firstY - HEX_1ST - 22, '1ST', 18, '#DDAE42')}
    ${leader1}
    ${follower1}

    ${rankLabel((secondLeaderX + secondFollowerX) / 2, secondY - HEX_RUNNER - 22, '2ND', 14, '#D5D5DE')}
    ${leader2}
    ${follower2}

    ${rankLabel((thirdLeaderX + thirdFollowerX) / 2, secondY - HEX_RUNNER - 22, '3RD', 14, '#D08F63')}
    ${leader3}
    ${follower3}
