// Template 09 — "SPOTLIGHT DUEL".
// 어두운 무대 좌우 천장에서 스포트라이트 두 개가 떨어지고, 가운데 은빛 사선 위에 크롬 "VS" —
// 두 참가자가 각자의 조명 아래 마주 서는 1:1 대결 무대.
// 잭앤질 스텝 화면·흐름은 07(LIVE ARENA)과 같고 09/common.ts 의 시각 프리미티브만 다르다.
...
const REPORT_THEME = {
  ...
  mono: DISPLAY,
  italic: false,
  // 09 은 상단 러너(대회명·아이콘·라이브 뱃지)가 y 84 까지 쓰고 하단 러너가 y 660 부터 시작한다.
  // 표를 그 사이(192~644)에 넣고, 좌우는 09 의 여백(32)에 맞춰 넓게 편다.
  layout: {
    leftX: 32,
    rightX: 652,
    colW: 596,
...
/**
 * 배경 레이어 확정.
 *  · 커스텀 배경이 없으면 09 기본 배경(벡터 연출)을 깐다 (그 아래 바탕 그라디언트는 shell 이 항상 깐다).
 *  · 있으면 기본 배경을 빼고 운영자 이미지 + 스크림을 깐다 — 09 은 흰 글자라
...
export const Template09: TemplateModule = {
  id: 9,
  name: 'Spotlight Duel — 듀얼 스포트라이트 (09)',
