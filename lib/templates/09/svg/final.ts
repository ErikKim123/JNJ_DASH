// Template 09 — 결승 전용 화면 (Prep / Wrapup / Result / Pairing).
  ...
  const mystery = `
    <g class="t07-mystery">
      ...
      <text class="num" x="${f(cx)}" y="${f(photoY + ph / 2 + (big ? 38 : 31))}" text-anchor="middle"
        font-family="'Montserrat', 'Arial Black', sans-serif" font-weight="900" font-size="${big ? 108 : 88}" fill="url(#t09AccentNum)" opacity="0.45">?<animate
...
export function finalResultSvg(): string {
  return shell(`
    ...
    <rect x="${CX - 0.75}" y="220" width="1.5" height="410" fill="url(#t09RuleV)"/>
    ${footBar()}
  `);
}
