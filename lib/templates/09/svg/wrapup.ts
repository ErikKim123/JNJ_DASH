// Template 09 — WRAPUP (집계 중). final=true 면 결승 변형(무대 조명 + 강조 띠).
...
const REEL_DEFS = `
  <defs>
    <linearGradient id="t09ReelFade" x1="0" y1="0" x2="0" y2="1">
      <stop offset="0" stop-color="#0B1C24" stop-opacity="1"/>
      <stop offset="0.14" stop-color="#0B1C24" stop-opacity="0.85"/>
      <stop offset="0.38" stop-color="#0B1C24" stop-opacity="0"/>
      <stop offset="0.62" stop-color="#0B1C24" stop-opacity="0"/>
      <stop offset="0.86" stop-color="#0B1C24" stop-opacity="0.85"/>
      <stop offset="1" stop-color="#0B1C24" stop-opacity="1"/>
    </linearGradient>
  </defs>
`;
...
function reel(x: number, y: number, w: number, h: number, idx: number, gold: boolean): string {
  ...
  for (let k = 0; k <= 10; k++) {
    digits += `<text x="${f(cx)}" y="${f(base + k * step)}" text-anchor="middle" font-family="${DISPLAY}" font-weight="900"
      font-size="${f(size)}" fill="url(#t09AccentNum)">${k % 10}</text>`;
  }
  ...
  return `
    ${panelCard(x, y, w, h, { rx: 16, hero: gold })}
    ${clipBox(x + 3, y + 3, w - 6, h - 6, strip)}
    <rect x="${f(x + 3)}" y="${f(y + 3)}" width="${f(w - 6)}" height="${f(h - 6)}" rx="13" fill="url(#t09ReelFade)"/>
