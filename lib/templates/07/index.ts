...
// 대회장 LED 월의 공식 라이브 디스플레이 그래픽 — 매젠타 조명이 번지는 딥 인디고 배경 위에
// 유리 카드 격자가 화면을 꽉 채우고, 흰색 초굵은 제목 + 황금 숫자로 정보를 전한다.
...
const REPORT_THEME = {
  ...
  mono: DISPLAY,
  italic: false,
  // 07 은 상단 러너(대회명·아이콘·라이브 뱃지)가 y 84 까지 쓰고 하단 러너가 y 660 부터 시작한다.
  // 표를 그 사이(192~644)에 넣고, 좌우는 07 의 여백(32)에 맞춰 넓게 편다.
  layout: {
    leftX: 32,
    rightX: 652,
    colW: 596,
...
/**
 * 배경 레이어 확정.
 *  · 커스텀 배경이 없으면 07 기본 배경 이미지를 깐다 (그 아래 벡터 그라디언트는 shell 이 항상 깐다).
 *  · 있으면 기본 이미지를 빼고 운영자 이미지 + 인디고 스크림을 깐다 — 07 은 흰 글자라
...
export const Template07: TemplateModule = {
  id: 7,
  name: 'Live Arena — 매젠타 라이브 디스플레이 (07)',
