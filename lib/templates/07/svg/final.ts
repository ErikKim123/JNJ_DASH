// Template 07 — 결승 전용 화면 (Prep / Wrapup / Result / Pairing).
import { prepSvg } from './prep';
import { wrapupSvg } from './wrapup';
import {
  shell, topBar, footBar, heading, glassCard, label, goldNumber, strong, rolePill, rankMark, photoFrame,
  clipBox, lightRays, sparkle, goldRule, fadeUp,
...
// 튀어 오르며(pop) 금빛 섬광과 함께 덮는다. TemplateRenderer 가 [data-reveal-id] 에
...
function seat(side: Role, x0: number, s: Seat): string {
  ...
  const mystery = `
    <g class="t07-mystery">
      ...
      <rect x="${f(x + 14)}" y="${f(photoY)}" width="${f(pw)}" height="${f(ph)}" rx="10" fill="none"
        stroke="${GOLD}" stroke-opacity="0.3" stroke-width="1.5" stroke-dasharray="7 7"/>
      <text class="num" x="${f(cx)}" y="${f(photoY + ph / 2 + (big ? 38 : 31))}" text-anchor="middle"
        font-family="'Montserrat', 'Arial Black', sans-serif" font-weight="900" font-size="${big ? 108 : 88}" fill="url(#t07GoldNum)" opacity="0.45">?<animate
  ...
  const card = `
    <g class="jnj-reveal" data-reveal-id="${side}-${rank}">
      ${glassCard(x, y, w, h, { gold: big })}
      ${rankMark(cx, rankY, rank, rankSize)}
      ${photoFrame(x + 14, photoY, pw, ph, `{{${prefix}_photo_${rank}}}`, { ring: big ? GOLD : FRAME, ringW: big ? 3 : 2.4 })}
...
export function finalResultSvg(): string {
  return shell(`
    ...
    <rect x="${CX - 0.75}" y="220" width="1.5" height="410" fill="url(#t07RuleV)"/>
    ${footBar()}
  `);
}
