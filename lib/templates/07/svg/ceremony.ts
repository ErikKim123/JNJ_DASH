// Template 07 — CEREMONY (결승 시상식).
// 순위별 유리 카드 3장이 화면 폭을 채운다 — 1위(중앙, 금테·가장 높음) · 2위(좌) · 3위(우).
...
import {
  ...
  WHITE, GOLD, GOLD_PALE, LEAD, FOLLOW, FRAME, f, type Role,
} from '../common';
...
function group(g: Group): string {
  ...
  const person = (px: number, role: Role, labelKey: string): string => {
    ...
    const pcx = px + pw / 2;
    return `
      ${photoFrame(px, photoY, pw, ph, `{{${prefix}_photo_${rank}}}`, { ring: big ? GOLD : FRAME, ringW: big ? 3 : 2.4 })}
  ...
  return fadeUp(
    g.delay,
    `
    ${glassCard(x, y, w, h, { rx: 22, gold: big })}
    ${big ? `${sparkle(cx - 78, rankY - 22, 10)}${sparkle(cx + 78, rankY - 22, 10)}` : ''}
    ${rankMark(cx, rankY, rank, rankSize)}
    ${label(cx, placeY, PLACE[rank], { size: big ? 13 : 11, tracking: big ? 6 : 4, fill: big ? GOLD : WHITE })}
...
const CONFETTI_COLORS = [GOLD, GOLD_PALE, LEAD, FOLLOW, '#FFFFFF', '#B99CFF'];
