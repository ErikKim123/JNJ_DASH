// Template 07 — WRAPUP (집계 중). final=true 면 결승 변형(무대 조명 + 금테 띠).
// 무대 띠 안에 슬롯머신처럼 굴러가는 황금 숫자 릴 7개 — "점수를 계산하는 중"을 글자 없이 보여준다.
// 릴마다 속도·시작 위치가 달라 기계적으로 똑같이 돌지 않는다.
import {
  shell, topBar, footBar, stageBand, glassCard, label, strong, clipBox, lightRays, seeded,
  DISPLAY, SOFT, GOLD, CX, MX, RX, f,
} from '../common';

const REEL_DEFS = `
  <defs>
    <linearGradient id="t07ReelFade" x1="0" y1="0" x2="0" y2="1">
      <stop offset="0" stop-color="#241B4C" stop-opacity="1"/>
      <stop offset="0.14" stop-color="#241B4C" stop-opacity="0.85"/>
      <stop offset="0.38" stop-color="#241B4C" stop-opacity="0"/>
      <stop offset="0.62" stop-color="#241B4C" stop-opacity="0"/>
      <stop offset="0.86" stop-color="#241B4C" stop-opacity="0.85"/>
      <stop offset="1" stop-color="#241B4C" stop-opacity="1"/>
    </linearGradient>
  </defs>
`;
...
function reel(x: number, y: number, w: number, h: number, idx: number, gold: boolean): string {
  ...
  for (let k = 0; k <= 10; k++) {
    digits += `<text x="${f(cx)}" y="${f(base + k * step)}" text-anchor="middle" font-family="${DISPLAY}" font-weight="900"
      font-size="${f(size)}" fill="url(#t07GoldNum)">${k % 10}</text>`;
  }
  ...
  return `
    ${glassCard(x, y, w, h, { rx: 16, gold })}
    ${clipBox(x + 3, y + 3, w - 6, h - 6, strip)}
    <rect x="${f(x + 3)}" y="${f(y + 3)}" width="${f(w - 6)}" height="${f(h - 6)}" rx="13" fill="url(#t07ReelFade)"/>
    <line x1="${f(x + 12)}" y1="${f(y + h * 0.2)}" x2="${f(x + w - 12)}" y2="${f(y + h * 0.2)}" stroke="${GOLD}" stroke-opacity="0.3"/>
    <line x1="${f(x + 12)}" y1="${f(y + h * 0.8)}" x2="${f(x + w - 12)}" y2="${f(y + h * 0.8)}" stroke="${GOLD}" stroke-opacity="0.3"/>
  `;
}
...
function progress(y: number): string {
  ...
  return `
    ...
    <rect x="${x1}" y="${y - 4}" width="${seg}" height="8" rx="4" fill="${GOLD}">
