...
import { shell, topBar, footBar, stageBand, strong, seeded, SOFT, GOLD, WHITE, CX, f, DISPLAY } from '../common';
...
function liveBadge(cx: number, cy: number): string {
  ...
  return `
    <g>
      ...
      <rect x="${x}" y="${y}" width="${w}" height="${h}" rx="${h / 2}" fill="url(#t07LiveRed)"/>
...
function equalizer(cx: number, base: number, maxH: number): string {
  ...
  for (let i = 0; i < N; i++) {
    ...
    const x = x0 + i * (bw + gap);
    bars += `
      <rect x="${f(x)}" y="${f(base - vals[0])}" width="${bw}" height="${f(vals[0])}" rx="3" fill="url(#t07Eq)">
