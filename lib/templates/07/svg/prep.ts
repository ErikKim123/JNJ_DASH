...
//     값이 없으면 카드 2장이 폭을 나눠 갖고 STAGE 가 금테 주인공 카드가 된다
//     (빈 금테 카드가 서 있으면 "뭔가 빠진 화면"으로 읽힌다).
// 카드 값은 모두 카드 안쪽 폭(좌우 30px 여백)에 맞춰 자동으로 줄어든다 — fit.ts.
import {
  shell, topBar, footBar, glassCard, label, strong, goldNumber, goldRule, fadeUp, lightRays,
  SOFT, GOLD, CX, MX, RX, f,
} from '../common';
...
function infoCard(x: number, c: Card, delay: number): string {
  ...
  const valueEl = c.gold
    ? goldNumber(cx, CARD_Y + 146, c.value, c.size, 'middle', maxW)
    : strong(cx, CARD_Y + 144, c.value, c.size, { weight: c.weight ?? 900, tracking: 0.5, fit: maxW, fitMin: 0.55 });
  return fadeUp(delay, `
    ${glassCard(x, CARD_Y, c.w, CARD_H, { gold: c.gold })}
