// Template 08 — "NEON VERSUS".
// 격투 게임 대결 화면의 키 비주얼 — 딥 네이비 위로 시안(좌상)과 마젠타(우하) 빛줄기가 사선으로 흐르고,
// 흰색 초굵은 이탤릭 글자가 두 색의 네온 글로우를 양쪽으로 머금는다.
// 잭앤질 스텝 화면·흐름은 07(LIVE ARENA)과 같고 08/common.ts 의 시각 프리미티브만 다르다.
...
const REPORT_THEME = {
  ...
  mono: DISPLAY,
  italic: false,
  // 08 은 상단 러너(대회명·아이콘·라이브 뱃지)가 y 84 까지 쓰고 하단 러너가 y 660 부터 시작한다.
  // 표를 그 사이(192~644)에 넣고, 좌우는 08 의 여백(32)에 맞춰 넓게 편다.
  layout: {
    leftX: 32,
    rightX: 652,
    colW: 596,
...
/**
 * 배경 레이어 확정.
 *  · 커스텀 배경이 없으면 08 기본 배경(벡터 연출)을 깐다 (그 아래 바탕 그라디언트는 shell 이 항상 깐다).
 *  · 있으면 기본 배경을 빼고 운영자 이미지 + 스크림을 깐다 — 08 은 흰 글자라
...
export const Template08: TemplateModule = {
  id: 8,
  name: 'Neon Versus — 사선 네온 대결 (08)',
