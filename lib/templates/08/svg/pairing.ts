...
import {
  shell, topBar, footBar, heading, neonCard, label, accentNumber, strong, roleChip, fadeUp, clipBox,
...
function renderCircle(pairCount: number): string {
  ...
  const center = `
    <g opacity="0">
      ...
      <circle cx="${f(L.cx)}" cy="${f(L.cy)}" r="${r}" fill="url(#t08CardFill)" stroke="url(#t08AccentStroke)" stroke-width="2.4"/>
