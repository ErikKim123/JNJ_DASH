// Template 08 — PREP (예선/본선 공통, final=true 면 결승 변형).
...
import {
  shell, topBar, footBar, neonCard, label, strong, accentNumber, accentRule, fadeUp, lightRays,
...
function infoCard(x: number, c: Card, delay: number): string {
  ...
  return fadeUp(delay, `
    ${neonCard(x, CARD_Y, c.w, CARD_H, { hero: c.gold })}
