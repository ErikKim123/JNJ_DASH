// Browser-side wrapper around the /api/vote/* routes. Same contract as the old
...
export async function getCompetitions(): Promise<Competition[]> {
  return call<Competition[]>('/api/vote/competitions');
}

export async function getJudges(competitionId?: string): Promise<Judge[]> {
  return call<Judge[]>(withQuery('/api/vote/judges', { competitionId }));
}

export async function getEvent(competitionId?: string): Promise<Event> {
  return call<Event>(withQuery('/api/vote/event', { competitionId }));
}

export async function getRound(round: Round, competitionId?: string, judgeId?: string): Promise<Contestant[]> {
  return call<Contestant[]>(withQuery('/api/vote/round', { round, competitionId, judgeId }));
}

export async function submitRound<R extends Round>(
  payload: SubmitPayload<R>,
  competitionId?: string,
  // finalize: true → mark judge submitted (admin column turns green),
  // false → clear it (Edit/unlock), undefined → leave as-is (intermediate SAVE).
  finalize?: boolean,
): Promise<{ written: number }> {
  return call<{ written: number }>('/api/vote/submit', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ ...payload, competitionId, finalize }),
  });
}
