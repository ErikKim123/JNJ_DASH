...
export const MESSAGES_EN = {
  ...
  'cf.judgesVideoHint': '📁 Browse = pick a local file (plays only when the display runs on this PC / localhost). ☁️ Upload = upload to cloud storage and play anywhere, including Vercel. Or paste a video https link / full local path manually. Only video files (mp4/webm/mov/m4v/mkv) are allowed. Leave empty to show a placeholder slide.',
  'cf.judgesVideoBrowse': 'Browse…',
  'cf.judgesVideoPicking': 'Opening…',
  'cf.judgesVideoUpload': 'Upload',
  'cf.judgesVideoUploading': 'Uploading…',
  'cf.judgesVideoUploadNeedSave': 'Save the contest first, then upload the video.',
  'cf.judgesVideoUploadFail': 'Video upload failed. If the file is too large, compress it or raise the Storage file-size limit in your Supabase dashboard.',
  'cf.judgesVideoBrowseFail': 'Could not open the file dialog. Make sure the local server (npm run dev) is running on this Windows PC at localhost:3000 (or 3001), then try again. Allow the browser prompt to access the local network if asked.',
...
export const MESSAGES_KO: Record<MessageKey, string> = {
  ...
  'cf.judgesVideoHint': '📁 파일 선택 = 로컬 파일 지정(표출을 이 PC/localhost에서 할 때만 재생). ☁️ 업로드 = 클라우드에 올려 Vercel 등 어디서든 재생. 또는 영상 https 링크/풀경로를 직접 붙여넣어도 됩니다. 영상파일(mp4/webm/mov/m4v/mkv)만 허용됩니다. 비워두면 "영상 미설정" 안내 슬라이드가 표출됩니다.',
  'cf.judgesVideoBrowse': '파일 선택…',
  'cf.judgesVideoPicking': '여는 중…',
  'cf.judgesVideoUpload': '업로드',
  'cf.judgesVideoUploading': '업로드 중…',
  'cf.judgesVideoUploadNeedSave': '대회를 먼저 저장한 뒤 영상을 업로드하세요.',
  'cf.judgesVideoUploadFail': '영상 업로드에 실패했습니다. 파일이 너무 크면 영상을 압축하거나, Supabase 대시보드에서 Storage 파일 크기 한도를 올리세요.',
  'cf.judgesVideoBrowseFail': '파일 선택 창을 열 수 없습니다. 이 PC에서 로컬 서버(npm run dev)가 localhost:3000(또는 3001)에 떠 있어야 합니다. 서버를 켠 뒤 다시 시도하고, 브라우저가 "로컬 네트워크 접근" 허용을 물으면 허용하세요.',
  // Tone & Manner (JOIN app theme)
